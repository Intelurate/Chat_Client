	...
	if(!window.PSLiveLinks) {
		...
		window.PSLiveLinks.getSearchresults = function() {
			...
			setTimeout(function() {	
				...
				window.PSLiveLinks.searchRecords = $('body').find('.article h2');				
		...
		window.PSLiveLinks.socket.on('showurlconnections', function (urls) {
			...
			if($('body').attr('swarm') != 'searched') {
				...
				for(var a = 0; a<window.PSLiveLinks.urls.length; a++) {													
					$('body').find('.article h2:eq('+a+')').prepend('<div class="googleSearchStat"><img src="'+chrome.extension.getURL("/images/icon.png")+'" /><p>'+urls[a].count+'</p></div>');
				}
			}else{
				for(var a = 0; a<window.PSLiveLinks.urls.length; a++) {													
					$('body').find('.article h2:eq('+a+')').prepend('<div class="googleSearchStat"><img src="'+chrome.extension.getURL("/images/icon.png")+'" /><p>'+urls[a].count+'</p></div>');
